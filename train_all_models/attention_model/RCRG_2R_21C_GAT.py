...
def main():
    ...
    # Non-temporal model: sec=False (single frame)
    ...
    training_config = TrainingConfig(
        model_name=getattr(config.training.group_activity, 'model_name', 'RCRG_2R_21C_GAT.pth'),
        num_classes=NUM_CLASSES,
        ...
        checkpoint_dir=f"{KAGGLE_OUTPUT}/checkpoints/RCRG_2R_21C_GAT" if IS_KAGGLE else config.training.group_activity.checkpoint_dir,
        log_dir=f"{KAGGLE_OUTPUT}/results/RCRG_2R_21C_GAT" if IS_KAGGLE else "reslutes_and_logs/attention_model",
        log_every_n_batches=getattr(config.training.group_activity, 'log_every_n_batches', 50),
