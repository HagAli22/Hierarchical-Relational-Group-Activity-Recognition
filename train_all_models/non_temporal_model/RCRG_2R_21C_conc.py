"""
RCRG-2R-21C-conc Training Script
============================================
Relational model with 2 Relational layers and 2+1 Cliques.
...
from models.non_temporal_model.RCRG_2R_21C_conc import RCRG_2R_21C_conc, collate_group_fn
from models import Person_Classifer
from train_utils.ddp_trainer import DDPTrainer, TrainingConfig


CONFIG_PATH = "configs/non_temporal_model/RCRG_2R_21C_conc.yaml"
...
def create_model():
    ...
    return RCRG_2R_21C_conc(person_model, num_classes=NUM_CLASSES)
...
def main():
    ...
    print("Loading RCRG_2R_21C_conc Configuration...")
    ...
    training_config = TrainingConfig(
        model_name=getattr(config.training.group_activity, 'model_name', 'RCRG_2R_21C_conc.pth'),
        ...
        checkpoint_dir=f"{KAGGLE_OUTPUT}/checkpoints/RCRG_2R_21C_conc" if IS_KAGGLE else config.training.group_activity.checkpoint_dir,
        log_dir=f"{KAGGLE_OUTPUT}/results/RCRG_2R_21C_conc" if IS_KAGGLE else "reslutes_and_logs/non_temporal_model",
