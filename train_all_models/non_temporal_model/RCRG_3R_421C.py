...
from models.non_temporal_model.RCRG_3R_421C import RCRG_3R_421C, collate_group_fn
from models import Person_Classifer
from train_utils.ddp_trainer import DDPTrainer, TrainingConfig


CONFIG_PATH = "configs/non_temporal_model/RCRG_3R_421C_config.yaml"
...
def create_model():
    ...
    return RCRG_3R_421C(person_model, num_classes=NUM_CLASSES)
...
def main():
    ...
    print("Loading RCRG_3R_421C Configuration...")
    ...
    training_config = TrainingConfig(
        model_name=getattr(config.training.group_activity, 'model_name', 'RCRG_3R_421C.pth'),
        ...
        checkpoint_dir=f"{KAGGLE_OUTPUT}/checkpoints/RCRG_3R_421C" if IS_KAGGLE else config.training.group_activity.checkpoint_dir,
        log_dir=f"{KAGGLE_OUTPUT}/results/RCRG_3R_421C" if IS_KAGGLE else "reslutes_and_logs/non_temporal_model",
