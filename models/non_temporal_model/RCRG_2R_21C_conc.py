"""
RCRG-2R-21C-conc Model
====================
Relational model with 2 Relational layers (2R) and 2+1 Cliques (21C).
...
class RCRG_2R_21C_conc(nn.Module):
    def __init__(self, person_classifier, num_classes=8, feature_dim=2048):
        super(RCRG_2R_21C_conc, self).__init__()
    ...
    def forward(self, x):  # x: (B, 12, 1, c, h, w)
        ...
        x = self.relation_layer1(x, clique_adjacency(K=12,num_cliques=2)) # (B, 12, 256)

        x= self.relation_layer2(x, clique_adjacency(K=12,num_cliques=1)) # (B, 12, 128)
