"""
RCRG-2R-11C-conc-Temporal Model
================================
Temporal relational model with 2 Relational layers (2R) and 1 Clique of all 12 players (11C).
Uses concatenation instead of max pooling and LSTM for temporal modeling across 9 frames.
"""
...
class RCRG_2R_11C_conc_Temporal(nn.Module):
    def __init__(self, person_classifier, num_classes=8, feature_dim=2048):
        ...
        # First relational layer: 4096 -> 128
    ...
    def forward(self, x):  # x: (B, 12, 9, c, h, w)
        ...
        # Create all pairs
        Pi = x.unsqueeze(2)  # (B*9, 12, 1, 2048)
        Pj = x.unsqueeze(1)  # (B*9, 1, 12, 2048)
        Pi_expanded = Pi.expand(-1, -1, k, -1)  # (B*9, 12, 12, 2048)
        Pj_expanded = Pj.expand(-1, k, -1, -1)  # (B*9, 12, 12, 2048)
...
class RCRG_2R_11C_conc_Temporalv1(nn.Module):
    def __init__(self, person_classifier, num_classes=8, feature_dim=2048):
        super(RCRG_2R_11C_conc_Temporalv1, self).__init__()
        ...
        # Second relational layer: 4096 -> 256
        self.relation_layer2 = nn.Sequential(
